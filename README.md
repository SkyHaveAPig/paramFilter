...
​	paramFilter 是一个用于筛选已选择的条件并以ajax访问后台接口的插件。
...
​		2）‘processor’是使用者提供的筛选数据处理方法，必填。
...
​	2.第二个参数‘ajaxItem’是插件发起ajax请求所需的参数，与jQuery插件的ajax请求参数相同，不需要填写回调方法与请求数据：
...
​	3.第三个参数‘callback’是插件的回调函数，会在每次触发事件时，规则处理全部执行之后执行，函数的第一个参数为筛选后的结果，可选。

​	4.第四个参数‘params’是插件的筛选值，要求值为‘对象数组’，内容可为一条或者多条，可选。

​	填写后将开启初次数据加载，以下是填写格式：
...
$('#example ul').filterData(
	...
	function(e){					//回调函数，'e'默认为选择的结果与结果所属的元素，当'ajaxItem'参数存在有效数据时，这											 里的'e'将为请求后台接口筛选后的值与结果所属的元素。
